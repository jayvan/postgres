...
#define F_LEAF				(1 << 0)	/* leaf page */ /*CS448 - for F_LEAF*/
...
// BEGIN CS448 FUNCTIONS & MACROS

static int32 _bt_prefixKeyCompress(Relation rel, IndexTuple lowItem, IndexTuple highItem);

#define ATTR_IS_TEXT(attr) (attr)->atttypid == 25

/*
 * Yanked from rtree.c's IndexTupleGetData. Surely it's useful for other
 * indexes, and deserves to be in a more general header file?!
 */
#define BTIndexTupleGetDatum(itup) \
            PointerGetDatum(((char *) (itup)) + sizeof(IndexTupleData))

/* debugging support routines */
void _btdump(Relation r);
void _btdumppage(Relation r, Buffer buf);

// END CS448 FUNCTIONS & MACROS
			   ...
			   bool split_only_page)
{
	...
	else
	{
		...
		END_CRIT_SECTION();
		  ...
		  /*
		       * BEGIN CS448 STUFF
		       * At this point on a page insert, the new entry is installed, and the
		       * page is pinned in the buffer pool, so it's a good time to print.
		       * Since we're concerned mostly with compressed keys on internal
		       * nodes, we don't print if this is a leaf node.
		       */
		      if (!(((BTPageOpaque)PageGetSpecialPointer(page))->btpo_flags & F_LEAF))
			 _btdumppage(rel, buf);
		      /* END CS448 STUFF */
...
/*
 * Prefix-key-compresses a key with respect to the key just below it in order.
 * This routine only tries to prefix-compress the first attribute of an index
 * key, and only if it is of type 'text'.
 *
 * Returns the number of characters that were compressed away.
 *
 * NOTE: This function may actually modify the passed-in highItem argument,
 * by changing the length of an embedded text structure's vl_len field!
 * Not intended to be called on the original copy of highItem!
 */
static int32
_bt_prefixKeyCompress(Relation rel, IndexTuple lowItem, IndexTuple highItem)
{
  TupleDesc itupdesc;
  Form_pg_attribute attr;

  /*
   * Datum is the "generic" Postgres internal representation for a value in
   * a column of a table
   */
  Datum lowDatum;
  Datum highDatum;
  ...
  /*
   * text is the Postgres internal representation for the SQL text type.
  ...
  itupdesc = RelationGetDescr(rel); /* description of the index keys in this index */
  attr = itupdesc->attrs[0]; /* description of the first attribute in the key */

  if(ATTR_IS_TEXT(attr)) {
    /* The first attribute of the index key is of SQL type text.  Let's compress! */

    /*
     * first, we get a pointer to the first attribute of the index key
     * in the high key (which we're compressing), and the next lowest key
     * (which tells us how to compress).
     */
    highDatum = BTIndexTupleGetDatum(highItem);
    lowDatum = BTIndexTupleGetDatum(lowItem);

    /* Now, we can cast the keys to type text */
    highText = (text *) highDatum;
    lowText = (text *) lowDatum;
     ...
     * highp so that it is as short as possible while still differentiating
     * the 2 subtrees.  Be careful not to walk off the end of either the
     * lowp or highp array! Remember that the length of the string in lowp
     * should be accessed by VARSIZE_ANY_EXHDR, and similar for highp.
     * When you get the right length, you need to update highText->vl_len
     * appropriately using SET_VARSIZE_SHORT.
     *
     * Also, you should save the difference between the old and new values
     ...
     */
    pos = sprintf(errbuf, "CS448 Saved %d chars, compressed to ", toReturn);

    for (i=0; i < VARSIZE_ANY_EXHDR(highText); i++)  	//remember vl_dat is 4 shorter!
		  ...
		  bool newitemonleft)
{
	...
	else
	{
	  ...
	  /* BEGIN INSERTED CODE FOR CS448 */

	  /*
	   * extract the highest key that will remain on the left page,
	   * i.e. the key below the split point
	   */
	  maxleftid = PageGetItemId(origpage, firstright - 1);
	  maxleftsz = ItemIdGetLength(maxleftid);
	  maxleft = (IndexTuple) PageGetItem(origpage, maxleftid);

	  /*
	   * We will want to prefix-compress a copy of the first key on the right page
	   * with respect to maxleft.  But be sure to use a COPY, or we'll mess up the
	   * key in the actual data entry in the leaf!
	   */
	  itemcpy = (IndexTuple)palloc(itemsz); /* always use palloc, NEVER malloc! */
	  memcpy((char *)itemcpy, (char *)item, itemsz); /* copy the item */
	  savings = _bt_prefixKeyCompress(rel, maxleft, itemcpy); /* compress it */

	  /*
	   * Assuming this worked right, itemcpy should now be prefix compressed
	   * in memory. As part of the Lehman/Yao concurrency control, we install
	   * this compressed version as the "high key" in the left-hand page.  Once
	   * installed there, the caller will copy the left-hand high-key up as the
	   * split key in the parent.
	   */
	  if (PageAddItem(leftpage, (Item) itemcpy, itemsz - savings, leftoff,
			  false,false) == InvalidOffsetNumber)
	  {
	  		memset(rightpage, 0, BufferGetPageSize(rbuf));
	  ...
	  /* be tidy -- call pfree on the copy once it's installed in the left */
	  pfree(itemcpy);

	  /* END INSERTED CS448 CODE */
...
/*
 * CS448 DEBUGGING SUPPORT
 * Dumps the contents of the b-tree index on the relation to the log.
 */
...
/*
 * CS448 DEBUGGING SUPPORT.
 * Note: this routine will not produce anything if
 * "buf" has not been to be pinned, since in that case it's not
 * really clear what you might find there.
 */
void _btdumppage(Relation r, Buffer buf)
{
  Page                  page;
  ...
  itupdesc = RelationGetDescr(r);
  /* This code written for the special case of a single-attribute index key
     of type text. */
  if ((itupdesc->natts > 1) || (!ATTR_IS_TEXT(itupdesc->attrs[0])))
    return;

  page = BufferGetPage(buf);
  /* this will only work if the caller has pinned the page in the buffer! */
  if (!BufferIsPinned(buf))
    return;

  blkno = BufferGetBlockNumber(buf);
  po = (BTPageOpaque) PageGetSpecialPointer(page);
  maxoff = PageGetMaxOffsetNumber(page);
  elog(DEBUG1, "Page %d maxoff %d <%s>\n", blkno, maxoff,
       (po->btpo_flags & F_LEAF ? "LEAF" : "INTERNAL"));
  ...
  for (offnum = FirstOffsetNumber;
    ...
    {
      if (ItemIdIsUsed(iid = PageGetItemId(page, offnum))) {
	itup = ((IndexTuple) PageGetItem(page, iid));
	itblkno = ItemPointerGetBlockNumber(&(itup->t_tid));
	if (offnum != P_FIRSTDATAKEY((BTPageOpaque)PageGetSpecialPointer(page))) {
	  itoffno = ItemPointerGetOffsetNumber(&(itup->t_tid));
	  datum = BTIndexTupleGetDatum(itup);
  	  itkey = VARDATA_ANY((text *)datum);
	  for(i = 0; i < VARSIZE_ANY_EXHDR((text *) datum) && i < sizeof(tmpbuf) - 1; i++)
	  	tmpbuf[i] = itkey[i];
	  tmpbuf[i] = '\0';
	  elog(DEBUG1, "\t[%d] size %d rid <%d,%d> key:%s\n",
	       offnum, IndexTupleSize(itup), itblkno, itoffno, tmpbuf);
	}
	else elog(DEBUG1, "\t[%d] size %d heap <%d,%d> (no key)\n",
	       offnum, IndexTupleSize(itup), itblkno, itoffno);
      }
    }
}
